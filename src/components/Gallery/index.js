...
class Gallery extends Component {
  ...
  choosenImage = id => {
    const imageEle = imagesList.filter(image => image.id === id)
    return imageEle[0]
  }
  ...
  render() {
    ...
    const imageEle = this.choosenImage(imageId)
    return (
      <div className="bgContainer">
        <div className="GalleryContainer">
          <img
            src={imageEle.imageUrl}
            alt={imageEle.imageAltText}
            className="mainImg"
          />
          ...
          <ul className="GalleryImages">
            {imagesList.map(eachImageDetails => (
              <ThumbnailItem
                ...
                activeClass={imageId === eachImageDetails.id}
              />
            ))}
          </ul>
        </div>
